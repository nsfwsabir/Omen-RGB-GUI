...
FanTab::FanTab(QWidget *parent) : QWidget(parent), m_fan(new FanController(this)) {
    ...
    auto *ctrlBox = new QGroupBox("Fan Control", this);
    auto *cLay = new QGridLayout(ctrlBox);
    m_maxFan = new QCheckBox("Max Fan (disables curve, keepalive 85s)", ctrlBox);
    cLay->addWidget(m_maxFan, 0,0,1,2);
    cLay->addWidget(new QLabel("Thermal Profile:", ctrlBox), 1,0);
    m_thermalProfile = new QComboBox(ctrlBox);
    m_thermalProfile->addItems(FanController::availableProfiles());
    m_thermalApply = new QPushButton("Apply", ctrlBox);
    auto *thRow = new QWidget(ctrlBox);
    ...
    cLay->addWidget(new QLabel("Temp Zone:", ctrlBox), 2,0);
    m_tempZone = new QComboBox(ctrlBox);
    ...
    m_tempZoneApply = new QPushButton("Apply", ctrlBox);
    auto *tzRow = new QWidget(ctrlBox);
    ...
    // Curve
    auto *curveBox = new QGroupBox("Fan Curve (2-8 points, temp 0-120°C : fan 0-100%, linear interp)", this);
    auto *curveMain = new QVBoxLayout(curveBox);
    auto *presetRow = new QHBoxLayout();
    presetRow->addWidget(new QLabel("Presets:", curveBox));
    m_presetCombo = new QComboBox(curveBox);
    ...
    auto *presetApply = new QPushButton("Load Preset", curveBox);
    ...
    m_curveWidget = new FanCurveWidget(curveBox);
    ...
    auto *info = new QLabel("Left-click drag points, Right-click remove, Double-click add (max 8, min 2). Values mapped via BIOS fan table.", curveBox);
    ...
    m_curveEdit = new QLineEdit(curveBox);
    m_curveEdit->setPlaceholderText("30:20 40:28 50:38 60:50 ...");
    m_curveApply = new QPushButton("Apply String", curveBox);
    m_curveReset = new QPushButton("Read Back", curveBox);
    curveCtrlRow->addWidget(new QLabel("String:", curveBox));
    ...
    m_curveEnable = new QCheckBox("Enable Fan Curve (requires Victus + BIOS table + CONFIG_THERMAL)", curveBox);
    auto *enableBtn = new QPushButton("Toggle Enable", curveBox);
    m_curveStringLabel = new QLabel("(unset)", curveBox);
...
void FanTab::pollRpm() {
    ...
    if (!cpuOk && !gpuOk) {
        // if fan interface NONE
        if (m_statusLabel->text().isEmpty()) m_statusLabel->setText("Fan interface not available (needs WMI + CONFIG_THERMAL)");
    }
}
...
void FanTab::applyMaxFan() {
    ...
    if (!m_fan->setMaxFan(en, &err)) {
        QMessageBox::critical(this,"Max Fan Failed", err);
...
void FanTab::applyThermalProfile() {
    ...
    if (!m_fan->setThermalProfile(prof, &err)) {
        QMessageBox::critical(this,"Thermal Profile Failed", err.isEmpty()? SysFs::lastError : err);
        return;
    }
...
void FanTab::applyTempZone() {
    ...
    if (!m_fan->setTempZone(zone, &err)) {
        QMessageBox::critical(this,"Temp Zone Failed", err);
        return;
    }
...
void FanTab::applyFanCurve() {
    ...
    if (!m_fan->setFanCurve(pts, &err)) {
        QMessageBox::critical(this,"Fan Curve Failed", err.isEmpty()? SysFs::lastError : err);
        return;
    }
...
void FanTab::presetSelected() {
    ...
    QString err;
    if (!m_fan->setFanCurve(pts, &err)) {
        QMessageBox::critical(this,"Preset Failed", err);
        return;
    }
    m_curveStringLabel->setText(FanController::pointsToString(pts));
}
...
void FanTab::toggleCurveEnable() {
    ...
    if (!m_fan->setCurveEnable(target, &err)) {
        QMessageBox::critical(this,"Curve Enable Failed", err.isEmpty()? SysFs::lastError : err + "\nRequires Victus + BIOS table + valid curve + max_fan off + CONFIG_THERMAL");
        return;
    }
